...
class CreativesController < ApplicationController
  ...
  def csv_import
    ...
    Creative.csv_import!(params[:file], current_user)
    imported_count = Creative.count - before_count
    if imported_count.zero?
      redirect_to company_creative_url(current_user.company), flash: { red: t('views.flash.fail_csv_data') }
    else
      redirect_to company_creative_url(current_user.company), flash: { green: t('views.flash.create_csv_data', count: imported_count) }
    end
  end
  ...
  def update
    if @creative.update(creative_params)
      redirect_to edit_company_creative_url(current_user.company, @creative), flash: { green: t('views.flash.update_success') }
    else
  ...
  def require_setting_csv_file
    redirect_to company_creatives_url(current_user.company), flash: { red: t('views.flash.non_csv_file') } unless params[:file].present? && File.extname(params[:file].original_filename) == '.csv'
  end
  ...
  def not_accessible_different_company_data
    redirect_to root_url, flash: { red: t('views.flash.not_accessible_different_company_data') } if current_user.company != @creative.company
  end

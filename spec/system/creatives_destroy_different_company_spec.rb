...
describe 'フロント画面：ログインアカウントと違う企業に紐づくCreativeの削除', type: :system do
  let!(:creative) { FactoryBot.create(:creative, name: '削除データ') }
  ...
  context 'ログインしているとき' do
    ...
    context '権限が grader のとき' do
      ...
      it '違う企業に紐づくCreativeの削除ボタンが存在しないこと' do
        expect(page).not_to have_content "delete_btn_#{creative.id}"
      end
    end
  end

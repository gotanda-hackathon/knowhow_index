...
describe 'フロント画面：ログインアカウントと同じ企業に紐づくCategoryの削除', type: :system do
  context 'ログインしているとき' do
    let!(:category) { FactoryBot.create(:category, name: '削除データ', company: login_user.company) }
    ...
    context '権限が grader のとき' do
      ...
      it '削除ボタンを押すと confirm アラートが出ること' do
        within "#delete_btn_#{category.id}" do
          click_on '削除'

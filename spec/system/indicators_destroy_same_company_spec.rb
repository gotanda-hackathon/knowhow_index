...
describe 'フロント画面：ログインアカウントと同じ企業に紐づくIndicatorの削除', type: :system do
  context 'ログインしているとき' do
    let!(:indicator) { FactoryBot.create(:indicator, name: '削除データ', company: login_user.company) }
    ...
    context '権限が grader のとき' do
      ...
      it '削除ボタンを押すと confirm アラートが出ること' do
        within "#delete_btn_#{indicator.id}" do
          click_on '削除'
      ...
      it '削除ボタンを押すとレコードが削除されていること' do
        ...
        aggregate_failures do
          ...
          expect(page).not_to have_content indicator.name
        end
      end
      ...
      it 'レコードが1件減ること' do
        ...
        end.to change(Indicator.all, :count).by(-1)
      end
    end
  end
